"""Opta Analyst provider.

Opta Analyst provides data-driven football analysis and predictions
...
class OptaAnalystProvider(BaseExpertProvider):
    """Provider for Opta Analyst predictions.

    Opta Analyst uses advanced statistics and data analysis
    to provide expert football predictions and insights.
    """

    BASE_URL = "https://theanalyst.com"
    FOOTBALL_URL = f"{BASE_URL}/football"
    PREDICTIONS_URL = f"{BASE_URL}/football/predictions"

    def __init__(self, rate_limit_delay: float = 3.0):
        """Initialize Opta Analyst provider.

        Args:
            rate_limit_delay: Seconds between requests (default: 3s)
        """
        super().__init__(
            source_name="Opta Analyst",
    ...
    async def fetch_latest_predictions(
        ...
        """Fetch latest predictions from Opta Analyst.
        ...
        # Fetch the predictions or football section
        ...
        prediction_articles = [
            ...
            if any(keyword in url.lower() for keyword in ['prediction', 'preview', 'analysis', 'odds'])
        ]
    ...
    def _extract_article_urls(self, soup, max_items: int) -> list[str]:
        ...
        # Find article links
        article_links = soup.find_all("a", href=re.compile(r"/football/.*"))
        ...
        for link in article_links[:max_items * 2]:
            ...
            if href not in urls and "theanalyst.com" in href:
                urls.append(href)
    ...
    async def _parse_article(self, url: str) -> list[ExpertPrediction]:
        ...
        # Extract summary
        summary_elem = (
            soup.find("p", class_=re.compile("intro|lead|summary"))
            or soup.find("meta", {"property": "og:description"})
        )
        summary = (
            summary_elem.get("content")
            if summary_elem and summary_elem.name == "meta"
            else summary_elem.get_text(strip=True) if summary_elem else None
        )

        # Extract author
        author = article_data.get("author", "Opta Analyst")
        ...
        for match_pred in match_predictions:
            try:
                ...
                # Add Opta-specific tags
                if match_pred.get("win_probability"):
                    match_tags["opta_probability"] = match_pred["win_probability"]
    ...
    def _extract_match_predictions(self, soup) -> list[dict]:
        ...
        # Opta Analyst patterns:
        # "Team A vs Team B: prediction (probability%)"
        # Often includes percentage-based predictions
        pattern1 = r'([A-Z][a-zA-Z\s&]+?)\s+(?:vs?\.?|v)\s+([A-Z][a-zA-Z\s&]+?)[\s:–-]+(?:Win probability:?\s*)?(\d+)%'
        ...
        for match in matches1:
            ...
            # Convert probability to pick (>50% = likely pick)
            if probability > 60:
                pick = '1'
            elif probability > 40:
                pick = '1X'
        ...
        pattern2 = r'([A-Z][a-zA-Z\s&]+?)\s+(?:vs?\.?|v)\s+([A-Z][a-zA-Z\s&]+?)[\s:–-]+(Home|Away|Draw)'
