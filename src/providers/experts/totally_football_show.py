"""The Totally Football Show provider.

The Totally Football Show is James Richardson's popular podcast featuring
expert analysis of matches and predictions from various football journalists.
"""
...
class TotallyFootballShowProvider(BaseExpertProvider):
    """Provider for The Totally Football Show.

    The Totally Football Show features James Richardson and a rotating cast of
    football journalists providing expert analysis and predictions.

    Note: This provider fetches articles. Podcast transcription requires ASR module.
    """

    BASE_URL = "https://www.theathletic.com"
    PODCAST_URL = f"{BASE_URL}/podcast/the-totally-football-show"

    def __init__(self, rate_limit_delay: float = 3.0):
        """Initialize The Totally Football Show provider.

        Args:
            rate_limit_delay: Seconds between requests (default: 3s)
        """
        super().__init__(
            source_name="The Totally Football Show",
    ...
    async def fetch_latest_predictions(
        ...
        """Fetch latest predictions from The Totally Football Show.
        ...
        # Find article/podcast episode links
        article_urls = self._extract_article_urls(soup, max_items)
        logger.info(f"Found {len(article_urls)} episodes/articles")

        # Filter for match prediction content
        prediction_articles = [
            url for url in article_urls
            if any(keyword in url.lower() for keyword in ['prediction', 'preview', 'tips'])
        ]
    ...
    def _extract_article_urls(self, soup, max_items: int) -> list[str]:
        """Extract article/podcast episode URLs.
    ...
    async def _parse_article(self, url: str) -> list[ExpertPrediction]:
        ...
        # Extract author
        author = article_data.get("author", "The Totally Football Show")
    ...
    def _extract_match_predictions(self, soup) -> list[dict]:
        ...
        # Find content area
        content = (
            soup.find("div", class_=re.compile("article-body|content"))
        ...
        # Pattern: "Team A v Team B: prediction"
