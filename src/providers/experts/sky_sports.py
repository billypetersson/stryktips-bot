...
Sky Sports provides expert analysis and predictions from renowned pundits
including Gary Neville and Jamie Carragher on Monday Night Football.
"""
...
class SkySportsProvider(BaseExpertProvider):
    """Provider for Sky Sports.

    Sky Sports features expert analysis from Gary Neville, Jamie Carragher,
    and other top pundits, particularly on Monday Night Football.
    ...
    PREDICTIONS_URL = f"{BASE_URL}/football/news"

    def __init__(self, rate_limit_delay: float = 3.0):
        """Initialize Sky Sports provider.
    ...
    async def fetch_latest_predictions(
        ...
        """Fetch latest predictions from Sky Sports.
        ...
        # Fetch the football news section
        html = await self._fetch_html(self.PREDICTIONS_URL)
        if not html:
            logger.error(f"Failed to fetch {self.PREDICTIONS_URL}")
            return []
        ...
        prediction_articles = [
            ...
            if any(keyword in url.lower() for keyword in [
                'prediction', 'preview', 'tips', 'neville', 'carragher', 'mnf',
                'monday-night-football', 'super-6'
            ])
        ]
    ...
    def _extract_article_urls(self, soup, max_items: int) -> list[str]:
        ...
        article_links = soup.find_all(
            "a",
            href=re.compile(r"/football/news/")
        )
    ...
    async def _parse_article(self, url: str) -> list[ExpertPrediction]:
        ...
        summary_elem = (
            ...
            or soup.find("p", class_=re.compile("standfirst|summary"))
        )
        if summary_elem:
            summary = (
        ...
        # Extract author
        author = article_data.get("author", "Sky Sports")
    ...
    def _extract_article_metadata(self, soup, url: str) -> Optional[dict]:
        ...
        # Extract author (often Neville, Carragher, or other pundits)
        author_elem = (
    ...
    def _extract_match_predictions(self, soup) -> list[dict]:
        ...
        # Sky Sports format: "Team A v Team B: prediction"
