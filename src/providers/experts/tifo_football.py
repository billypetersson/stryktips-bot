"""Tifo Football provider.

Tifo Football provides tactical analysis videos and articles with in-depth
...
class TifoFootballProvider(BaseExpertProvider):
    """Provider for Tifo Football.

    Tifo Football is known for its tactical analysis videos and articles,
    providing detailed match previews and predictions.

    Note: This provider fetches articles. Video transcription requires ASR module.
    """

    BASE_URL = "https://www.tifofootball.com"
    ARTICLES_URL = f"{BASE_URL}/articles"

    def __init__(self, rate_limit_delay: float = 3.0):
        """Initialize Tifo Football provider.

        Args:
            rate_limit_delay: Seconds between requests (default: 3s)
        """
        super().__init__(
            source_name="Tifo Football",
    ...
    async def fetch_latest_predictions(
        ...
        """Fetch latest predictions from Tifo Football.
        ...
        # Find article/video links
        article_urls = self._extract_article_urls(soup, max_items)
        logger.info(f"Found {len(article_urls)} articles/videos")

        # Filter for match prediction content
        prediction_articles = [
            url for url in article_urls
            if any(keyword in url.lower() for keyword in ['prediction', 'preview', 'tactical', 'analysis'])
        ]
    ...
    async def _parse_article(self, url: str) -> list[ExpertPrediction]:
        ...
        # Extract author
        author = article_data.get("author", "Tifo Football")
    ...
    def _extract_article_metadata(self, soup, url: str) -> Optional[dict]:
        ...
        author_elem = (
            ...
            or soup.find("span", class_=re.compile("author"))
        )
        if author_elem:
            metadata["author"] = (
    ...
    def _extract_match_predictions(self, soup) -> list[dict]:
        ...
        # Tifo format can vary, but typically: "Team A vs Team B: prediction" or "Team A - Team B"
        pattern = r'([A-Z][a-zA-Z\s&]+?)\s+(?:vs?|v\.?|-)\s+([A-Z][a-zA-Z\s&]+?)(?::\s*)?(Home win|Away win|Draw|Win|[\dX\-]{1,3})'
    ...
    def _convert_prediction_to_pick(self, prediction: str) -> Optional[str]:
        ...
        if any(term in pred_lower for term in ['home win', 'home victory', 'home to win']):
            return '1'
        elif any(term in pred_lower for term in ['away win', 'away victory', 'away to win']):
            return '2'
